...
####### generate functional response in Case1
sigma1 <- 0.01*sin(0.1*pi0*x)+0.01
sigma2 <- 0.01*cos(0.1*pi0*x)+0.01

error13 <- matrix(0,n1,break_p)
error23 <- matrix(0,n2,break_p)
for(l in 1:break_p){
  error13[,l] <- rnorm(n1,0,sigma1[l])
  error23[,l] <- rnorm(n2,0,sigma2[l])
}
Y1 <- matrix(0,n,break_p)
sig11 <- x_ilr[index1,] %*% t(data_beta_ilr1) + matrix(z[index1] %X%gama_data1,break_p,n1)
sig12 <- x_ilr[index2,] %*% t(data_beta_ilr2) + matrix(z[index2] %x%gama_data2,break_p,n2)
Y1[index1,] <- sig11+error13
Y1[index2,] <- sig12+error23
