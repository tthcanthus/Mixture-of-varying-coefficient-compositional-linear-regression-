...
####### generate functional response in Case3
error11  <- kronecker(rnorm(n1,0,sqrt(0.04)),sqrt(2)*sin(4*pi0*x)+
  kronecker(rnorm(n1,0,sqrt(0.01)),sqrt(2)*cos(4*pi0*x))
error11 <- t(matrix(error11,break_p,n1))+e[index1]

error21  <- kronecker(rnorm(n2,0,sqrt(0.01)),sqrt(2)*sin(3*pi0*x))+
  kronacker(rnorm(n2,0,sqrt(0.04)),sqrt(2)*cos(3*pi0*x))
error21 <- t(matrix(error21,break_p,n2))+e[index2]

Y3 <- matrix(0,n,break_p)
sig31 <- x_ilr[index1,] %*% t(data_beta_ilr1) + matrix(z[index1] %X%gama_data1,break_p,n1)
sig32 <- x_ilr[index2,] %*% t(data_beta_ilr2) + matrix(z[index2] %x%gama_data2,break_p,n2)
Y3[index1,] <- sig31 + error11
Y3[index2,] <- sig32 + error21
