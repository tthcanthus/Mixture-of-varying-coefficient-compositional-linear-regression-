...
min.dis <- min(sum((MResult1$coef[[1]][1,]-com_beta1[,1])^2),sum((MResult1$coef[[2]][1,]-com_beta1[,1])^2))
index.min <- which(c(sum((MResult1$coef[[1]][1,]-com_beta1[,1])^2),sum((MResult1$coef[[2]][1,]-com_beta1[,1])^2))==min.dis)

max.dis <- max(sum((MResult1$coef[[1]][1,]-com_beta1[,1])^2),sum((MResult1$coef[[2]][1,]-com_beta1[,1])^2))
index.max <- which(c(sum((MResult1$coef[[1]][1,]-com_beta1[,1])^2),sum((MResult1$coef[[2]][1,]-com_beta1[,1])^2))==max.dis)

plot(Data2fd(t,as.matrix(cbind(com_beta1,gama_data1))),
     col=1,ylim=c(-1,1))
lines(t,t(MResult1$coef[[index.min]])[,1],col=2)
lines(t,t(MResult1$coef[[index.min]])[,2],col=2)
lines(t,t(MResult1$coef[[index.min]])[,3],col=2)

plot(Data2fd(t,as.matrix(cbind(com_beta2,gama_data2))),
     col=1,ylim=c(-1,1))
lines(t,t(MResult1$coef[[index.max]])[,1],col=2)
lines(t,t(MResult1$coef[[index.max]])[,2],col=2)
lines(t,t(MResult1$coef[[index.max]])[,3],col=2)
